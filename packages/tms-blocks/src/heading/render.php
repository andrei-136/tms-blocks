...
if ( ! defined( 'ABSPATH' ) ) {
    ...
    $span_class   = $attributes['spanClass'] ?? 'letter';
    ...
    if ( $is_dynamic ) {
        ...
        if ( $dynamic_path === '' ) {
            $heading_content = $empty_text ? wp_kses( $empty_text, $allowed_tags ) : '';
        } else {
        ...
        $heading_content    = wp_kses( $heading_content, $block_content_tags );
    }
    ...
    $output = "<{$tag}{$attrs}>{$heading_content}</{$tag}>";
